...
class CategoryController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        // $category->descriptions = $request->descriptions;
        if ($category->save()) {
            return redirect()->route('backend.category.index')->with(['alert-type' => 'success', 'message' => 'Category Updated Successfully']);
        }
        return redirect()->back()->with(['alert-type' => 'error', 'message' => 'Something Went Wrong']);
    }
    ...
    public function destroy($id)
    {
        ...
        if ($category->products()->exists() || $category->subCategories()->exists()) {
            return redirect()->back()->with(['alert-type' => 'error', 'message' => 'Product / Sub Category exists in this Category']);
        }
        if ($category->delete()) {
